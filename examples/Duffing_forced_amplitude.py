import sys
import matplotlib.pyplot as plt

filepath = 'D:\PhD study\simplines-main\simplines/'
sys.path.append(filepath)
import bsplines as bs
import torch
from torch import autograd
from scipy.integrate import odeint
import numpy as np
import random
import math

a = -5
b = -0.5
c = -10
amp = 4
w = 0.6 * np.pi


## Generate data by solving ODE
def pfun(x, t):
    x1, x2 = x
    a = -5
    b = -0.5
    c = -10
    amp = 4
    w = 0.6 * np.pi
    return np.array([x2, a * x1 + b * x2 + c * x1 ** 3 + amp * np.cos(w * t)])


del_t = 0.2
T = 10 + del_t
t = np.arange(0, T, del_t)
sol = odeint(pfun, [1, 1], t)
# Add noise into solution
sol1 = sol[:, 0]
sol2 = sol[:, 1]
w1 = np.var(sol1)
w2 = np.var(sol2)
w3 = np.var(a * sol1 + b * sol2 + c * sol1 ** 3 + amp * np.cos(w * t))
sol3 = a * sol1 + b * sol2 + c * sol1 ** 3 + amp * np.cos(w * t)
m = 0
sigma = 0.0
for i in range(len(sol)):
    sol1[i] += random.gauss(m, sigma)
    sol2[i] += random.gauss(m, sigma)
plt.plot(sol1)
plt.plot(sol2)
plt.show()

## Generate B-spline basis matrix
# The last entry of every b at location x represents the value of the span-th basis function at x
degree = 3
del_tb = random.uniform(del_t / degree, del_t / (degree + 1))
knot = bs.make_knots(np.linspace(0, T - del_t, num=math.ceil(T / del_tb)), degree, False)

x = [del_t * i for i in range(1, len(sol1) - 1)]
B = np.zeros((len(x), len(knot) - 1 - degree - 2 * degree))
B_d = np.zeros((len(x), len(knot) - 1 - degree - 2 * degree))
B_2d = np.zeros((len(x), len(knot) - 1 - degree - 2 * degree))
for i in range(0, len(x)):
    span = bs.find_span(knot, degree, x[i])
    b = bs.basis_funs_all_ders(knot, degree, x[i], span, 2)
    for j in range(0, degree + 1):
        B[i, span - 2 * degree + j] = b[0, j]
        B_d[i, span - 2 * degree + j] = b[1, j]
        B_2d[i, span - 2 * degree + j] = b[2, j]

## Generate new B for ODE
x_O = np.arange(del_t, T - 2 * del_t + del_t / 4, del_t / 4)
B_O = np.zeros((len(x_O), len(knot) - 1 - degree - 2 * degree))
B_d_O = np.zeros((len(x_O), len(knot) - 1 - degree - 2 * degree))
B_2d_O = np.zeros((len(x_O), len(knot) - 1 - degree - 2 * degree))
for i in range(0, len(x_O)):
    span = bs.find_span(knot, degree, x_O[i])
    b = bs.basis_funs_all_ders(knot, degree, x_O[i], span, 2)
    for j in range(0, degree + 1):
        B_O[i, span - 2 * degree + j] = b[0, j]
        B_d_O[i, span - 2 * degree + j] = b[1, j]
        B_2d_O[i, span - 2 * degree + j] = b[2, j]

## Define the exact line search algorithm
# def Adam(Bbeta, beta, B_O, B_d_O, B_2d_O, theta2, theta3, mu, tt, w1, x1):
#     gamma = torch.tensor(0.5, dtype=torch.float64)
#     gamma.requires_grad_()
#     beta1 = 0.9
#     beta2 = 0.999
#     epsilon = 10e-8
#     alpha = 0.001
#     m_j = 0
#     v_j = 0
#     gbeta = beta + gamma * (Bbeta - beta)
#     threshold = 1e-12
#     F2 = torch.cat(
#         [torch.cat([torch.cat([B_O @ gbeta, B_d_O @ gbeta], 1), (B_O @ gbeta) * (B_O @ gbeta) * (B_O @ gbeta)], 1),
#          torch.cos(tt * theta3)], 1)
#     f = mu / 2 * torch.norm(B_2d_O @ beta - F2 @ theta2)+ gamma * 1 / w1 * (
#             torch.norm(x1[1:len(x1) - 1] - B @ Bbeta) - torch.norm(x1[1:len(x1) - 1] - B @ beta))
#     gamma_grads = autograd.grad(outputs=f, inputs=gamma)[0]
#     m_j = beta1 * m_j + (1 - beta1) * gamma_grads
#     v_j = beta2 * v_j + (1 - beta2) * gamma_grads ** 2
#     alpha_j = alpha * np.sqrt(1 - beta2) / (1 - beta1)
#     gamma_new = gamma - alpha_j * m_j / (np.sqrt(v_j) + epsilon * np.sqrt(1 - beta2))
#     j = 2
#     while (1):
#         if torch.abs(gamma_new - gamma) > threshold:
#             gamma = gamma_new
#             gbeta = beta + gamma * (Bbeta - beta)
#             F2 = torch.cat(
#                 [torch.cat([torch.cat([B_O @ gbeta, B_d_O @ gbeta], 1), (B_O @ gbeta) * (B_O @ gbeta) * (B_O @ gbeta)],
#                            1),
#                  torch.cos(tt * theta3)], 1)
#             f = mu / 2 * torch.norm(B_2d_O @ beta - F2 @ theta2) + gamma * 1 / w1 * (
#                     torch.norm(x1[1:len(x1) - 1] - B @ Bbeta) - torch.norm(x1[1:len(x1) - 1] - B @ beta))
#             gamma_grads = autograd.grad(outputs=f, inputs=gamma)[0]
#             m_j = beta1 * m_j + (1 - beta1) * gamma_grads
#             v_j = beta2 * v_j + (1 - beta2) * gamma_grads ** 2
#             alpha_j = alpha * np.sqrt(1 - beta2 ** j) / (1 - beta1 ** j)
#             gamma_new = gamma - alpha_j * m_j / (np.sqrt(v_j) + epsilon * np.sqrt(1 - beta2 ** j))
#             # print(gamma_grads,gamma_new)
#             j += 1
#             if gamma_new > 1:
#                 gamma_new = 1
#                 break
#             elif gamma_new < 0:
#                 gamma_new = 0.01
#                 break
#         else:
#             break
#     print(gamma_grads,gamma_new)
#     return gamma_new


## Initialization
beta = torch.rand(B.shape[1], 1)
beta = torch.tensor(beta, dtype=torch.float64)
theta2 = torch.rand(4, 1)
theta2 = torch.tensor(theta2, dtype=torch.float64)
theta3 = torch.tensor(0.6 * np.pi, dtype=torch.float64)

beta.requires_grad_()
theta2.requires_grad_()

B = torch.from_numpy(B)
B_d = torch.from_numpy(B_d)
B_2d = torch.from_numpy(B_2d)
B_O = torch.from_numpy(B_O)
B_d_O = torch.from_numpy(B_d_O)
B_2d_O = torch.from_numpy(B_2d_O)
tt = torch.rand(len(x_O), 1)
for i in range(0, len(x_O)):
    tt[i] = x_O[i]
tt = torch.tensor(tt, dtype=torch.float64)
x1 = torch.rand(len(sol1), 1)
x2 = torch.rand(len(sol2), 1)
x3 = torch.rand(len(sol3), 1)
for i in range(0, len(sol)):
    x1[i] = sol1[i]
    x2[i] = sol2[i]
    x3[i] = sol3[i]
x1 = torch.tensor(x1, dtype=torch.float64)
x2 = torch.tensor(x2, dtype=torch.float64)
x3 = torch.tensor(x3, dtype=torch.float64)
## Iteration of Successive Convex Approximation
maxiter = 200000
mu = 300
c = 500
gamma = 1
epsilon = 0.99
gamma1 = 1
a1 = 0
a2 = 0
a3 = 500
for i in range(0, maxiter + 1):

    if i == 0:
        # Compute the approximated solution of beta
        F2 = torch.cat(
            [torch.cat([torch.cat([B_O @ beta, B_d_O @ beta], 1), (B_O @ beta) * (B_O @ beta) * (B_O @ beta)], 1),
             torch.cos(tt * theta3)], 1)
        f = mu / 2 * torch.norm(B_2d_O @ beta - F2 @ theta2)
        f_grads = autograd.grad(outputs=f, inputs=beta, retain_graph=1)[0]

        Bbeta = (torch.inverse(
            a1 * 1 / w1 * B.T @ B + a2 * 1 / w2 * B_d.T @ B_d + a3 * 1 / w3 * B_2d.T @ B_2d + c * np.eye(
                B.shape[1]))) @ (
                        a1 * 1 / w1 * B.T @ x1[1:len(x1) - 1] + a2 * 1 / w2 * B_d.T @ x2[1:len(
                    x2) - 1] + a3 * 1 / w3 * B_2d.T @ x3[1:len(x3) - 1] + c * beta - f_grads)
        gamma = gamma * (1 - gamma * epsilon)
        # gamma=Adam(Bbeta, beta, B_O, B_d_O, B_2d_O, theta2, theta3, mu, tt, w1, x1)
        with torch.no_grad():
            beta += gamma * (Bbeta - beta)

        # Update theta1 and theta2
        with torch.no_grad():
            theta2.copy_(torch.inverse(F2.T @ F2) @ F2.T @ (torch.matmul(B_2d_O, beta)))

        # Print iteration and residual of each term
        fit_res = a1 * torch.norm(x1[1:len(x1) - 1] - B @ beta) + a2 * torch.norm(
            x2[1:len(x2) - 1] - B_d @ beta) + a3 * torch.norm(
            x3[1:len(x3) - 1] - B_2d @ beta)
        ode_res = torch.norm(B_2d_O @ beta - F2 @ theta2)
        res = fit_res + ode_res
        print('Iteration=%d, Fitting residual=%f, ODE residual=%f, Total residual=%f' % (
            i, fit_res, ode_res, res))
        print(
            ',Theta2=' + str(
                theta2.detach().numpy().T) + '\n')
    else:
        # Compute the approximated solution of beta
        with torch.no_grad():
            F2 = torch.cat(
                [torch.cat([torch.cat([B_O @ beta, B_d_O @ beta], 1), (B_O @ beta) * (B_O @ beta) * (B_O @ beta)], 1),
                 torch.cos(tt * theta3)], 1)
        f = mu / 2 * torch.norm(B_2d_O @ beta - F2 @ theta2)
        with torch.no_grad():
            f_grads.copy_(autograd.grad(outputs=f, inputs=beta, retain_graph=1)[0])

            Bbeta.copy_((torch.inverse(
                a1 * 1 / w1 * B.T @ B + a2 * 1 / w2 * B_d.T @ B_d + a3 * 1 / w3 * B_2d.T @ B_2d + c * np.eye(
                    B.shape[1]))) @ (
                                a1 * 1 / w1 * B.T @ x1[1:len(x1) - 1] + a2 * 1 / w2 * B_d.T @ x2[1:len(
                            x2) - 1] + a3 * 1 / w3 * B_2d.T @ x3[1:len(x3) - 1] + c * beta - f_grads))
        gamma = gamma * (1 - gamma * epsilon)
        with torch.no_grad():
            beta += gamma * (Bbeta - beta)

            # Update theta1 and theta2
            theta2.copy_(torch.inverse(F2.T @ F2) @ F2.T @ (torch.matmul(B_2d_O, beta)))

        # Print iteration and residual of each term
        fit_res = a1 * torch.norm(x1[1:len(x1) - 1] - B @ beta) + a2 * torch.norm(
            x2[1:len(x2) - 1] - B_d @ beta) + a3 * torch.norm(
            x3[1:len(x3) - 1] - B_2d @ beta)
        ode_res = torch.norm(B_2d_O @ beta - F2 @ theta2)
        res = fit_res + ode_res
        if i % 100 == 0:
            print('Iteration=%d, Fitting residual=%f, ODE residual=%f, Total residual=%f' % (
                i, fit_res, ode_res, res))
            print(
                ',Theta2=' + str(
                    theta2.detach().numpy().T) + '\n')

# Plot figures to check results
plt.scatter(x, sol1[1:len(sol1) - 1], label='Measurements of x1', c='r')
# plt.plot(x, sol1[1:len(sol1) - 1], label='Ture x1', c='g')
y1 = torch.matmul(B_O, beta)
plt.plot(x_O, y1.detach().numpy(), label='Fitting x1')
plt.legend(loc="best", fontsize=10)
plt.show()

plt.scatter(x, sol2[1:len(sol1) - 1], label='Ture x2', c='g')
y2 = torch.matmul(B_d_O, beta)
plt.plot(x_O, y2.detach().numpy(), label='Fitting x2')
plt.legend(loc="best", fontsize=10)
plt.show()

plt.scatter(x, sol3[1:len(sol) - 1], label='Ture x3', c='g')
y3 = torch.matmul(B_2d_O, beta)
plt.plot(x_O, y3.detach().numpy(), label='Fitting x3')
plt.legend(loc="best", fontsize=10)
plt.show()

ODE = B_2d_O @ beta - F2 @ theta2
plt.plot(ODE.detach().numpy(), label='ODE residual')
plt.legend(loc="best", fontsize=10)
plt.show()
