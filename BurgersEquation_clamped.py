...
Bt = np.zeros((len(t1), len(knot_t) - 1 - degree_t))

f_batch_size = 2 ** 14  # The number of points put into algorithm in each batch, so the size of B_P is fixed

Bt_P = np.zeros((f_batch_size, len(knot_t) - 1 - degree_t))
for i in range(0, len(t1)):
    ...
    for j in range(0, degree_t + 1):
        Bt[i, span - degree_t + j] = b[0, j]

del_x = 0.167  # The distance between sensors
X = 5 + del_x  # The distance between the first and the last sensor
degree_x = 3
# del_xb = random.uniform(del_x / degree_x, del_x / (degree_x + 1))
del_xb = 0.04
knot_x = bs.make_knots(np.linspace(0, X - del_x, num=math.ceil((X + 0) / del_xb)), degree_x, False)

Bx = np.zeros((len(x1), len(knot_x) - 1 - degree_x))
for i in range(0, len(x1)):
    ...
    for j in range(0, degree_x + 1):
        Bx[i, span - degree_x + j] = b[0, j]
Bx_P = np.zeros((f_batch_size, len(knot_x) - 1 - degree_x))
...
for k in range(0, len(x0)):
    ...
    for m in range(0, degree_x + 1):
        for n in range(0, degree_t + 1):
            B[k, (span_x - degree_x + m) * Bt.shape[1] + span_t - degree_t + n] = b_x[0, m] * b_t[0, n]
...
def exact_line_search(B, B_P, B_x_P, B_t_P, B_2x_P, u, Bbeta, beta, theta, mu, w1, a1):
    ...
    a = 4 * 0.5 * mu * B1.T @ B1
    b = 3 * 0.5 * mu * (B1.T @ B2 + B2.T @ B1)
    c = 2 * 0.5 * mu * (B1.T @ B3 + B2.T @ B2 + B3.T @ B1)
    d = 0.5 * mu * (B2.T @ B3 + B3.T @ B2) + g
...
# Operate SCA
n_epoch = 2000  # The number of epoch
...
for i in range(0, n_epoch):
    ...
    # for j in range(0, int(num_points / f_batch_size)):
        ...
        for k in range(0 + f_batch_size * j, f_batch_size * (j + 1)):
            ...
            for m in range(0, degree_x + 1):
                for n in range(0, degree_t + 1):
                    B_P[k - f_batch_size * j, (span_x - degree_x + m) * Bt.shape[1] + span_t - degree_t + n] = \
                        b_x[0, m] * b_t[0, n]
                    B_t_P[k - f_batch_size * j, (span_x - degree_x + m) * Bt.shape[1] + span_t - degree_t + n] = \
                        b_x[0, m] * b_t[1, n]
                    B_x_P[k - f_batch_size * j, (span_x - degree_x + m) * Bt.shape[1] + span_t - degree_t + n] = \
                        b_x[1, m] * b_t[0, n]
                    B_2x_P[
                        k - f_batch_size * j, (span_x - degree_x + m) * Bt.shape[1] + span_t - degree_t + n] = \
                        b_x[2, m] * b_t[0, n]
...
for k in range(0, len(x_coords_d)):
    ...
    for m in range(0, degree_x + 1):
        for n in range(0, degree_t + 1):
            B_Pd[k, (span_x - degree_x + m) * Bt.shape[1] + span_t - degree_t + n] = \
                b_x[0, m] * b_t[0, n]
            B_t_Pd[k, (span_x - degree_x + m) * Bt.shape[1] + span_t - degree_t + n] = \
                b_x[0, m] * b_t[1, n]
            B_x_Pd[k, (span_x - degree_x + m) * Bt.shape[1] + span_t - degree_t + n] = \
                b_x[1, m] * b_t[0, n]
            B_2x_Pd[
                k, (span_x - degree_x + m) * Bt.shape[1] + span_t - degree_t + n] = \
                b_x[2, m] * b_t[0, n]
...
plt.plot(np.linspace(0,1,len(beta)),beta.detach().numpy())
plt.plot(np.linspace(0,1,len(u)),u)
plt.show()

zz3 = (B_t_Pd @ beta + (B_Pd @ beta) * (B_x_Pd @ beta) - theta * B_2x_Pd@beta).detach().numpy()
ZZ3 = np.zeros((len(t1_P), len(x1_P)))
for i in range(0, len(x1_P)):
    ZZ3[:, i] = zz3[i * len(t1_P):(i + 1) * len(t1_P), 0]
fig = go.Figure(data=[go.Surface(
