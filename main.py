...
w1 = np.var(sol1) / 80
w2 = np.var(sol2)/ 500
w3 = np.var(-10 * sol1 - 0.5 * sol2 - 30 * sol1 ** 3)
m = 0
sigma = 0.1
...
for i in range(0, maxiter + 1):
    ...
    # def step_gradient(gamma, Btheta3, learningRate, beta, B_O, B_d_O, B_2d_O, theta1, theta2, mu, w3, tt):
    #     gtheta3 = theta3 + gamma * (Btheta3 - theta3)
    #     B_beta3 = torch.matmul(B_O, beta) * torch.matmul(B_O, beta) * torch.matmul(B_O, beta)
    #     F2 = torch.cat([torch.cat([torch.matmul(B_O, beta), torch.matmul(B_d_O, beta)], 1), B_beta3], 1)
    #     F1 = torch.cos(tt * gtheta3)
    #     f = mu / 2 / w3 * torch.norm(torch.matmul(B_2d_O, beta) - torch.matmul(F2, theta2) + theta1 * F1)
    ...
    #
    #
    # gamma1 = torch.tensor(0.5, dtype=torch.float64)
    # gamma1.requires_grad_()
    # while (1):
    #     gamma1_new = step_gradient(gamma1, Btheta3, 1e-40, beta, B_O, B_d_O, B_2d_O, theta1, theta2, mu, w3, tt)
    #     if torch.abs(gamma1_new - gamma1) >1e-40:
    #         gamma1=gamma1_new
    #     else:
    #         break
